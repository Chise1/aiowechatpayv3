...
def applyment_submit(
    ...
    if params.get("contact_info").get("contact_name"):
        params["contact_info"]["contact_name"] = self._core.encrypt(
            params["contact_info"]["contact_name"]
        )
    if params.get("contact_info").get("contact_id_number"):
        params["contact_info"]["contact_id_number"] = self._core.encrypt(
    ...
    if params.get("contact_info").get("mobile_phone"):
        params["contact_info"]["mobile_phone"] = self._core.encrypt(
            params["contact_info"]["mobile_phone"]
        )
    if params.get("contact_info").get("contact_email"):
        params["contact_info"]["contact_email"] = self._core.encrypt(
            params["contact_info"]["contact_email"]
        )
    id_card_name = (
        params.get("subject_info").get("identity_info").get("id_card_info", {}).get("id_card_name")
    )
    if id_card_name:
        params["subject_info"]["identity_info"]["id_card_info"][
            "id_card_name"
        ] = self._core.encrypt(id_card_name)
    id_card_number = (
        params.get("subject_info")
        .get("identity_info")
        .get("id_card_info", {})
        .get("id_card_number")
    )
    if id_card_number:
        params["subject_info"]["identity_info"]["id_card_info"][
            "id_card_number"
        ] = self._core.encrypt(id_card_number)
    id_card_address = (
        params.get("subject_info")
        .get("identity_info")
        .get("id_card_info", {})
    ...
    if id_card_address:
        params["subject_info"]["identity_info"]["id_card_info"][
            "id_card_address"
        ] = self._core.encrypt(id_card_address)
    id_doc_name = (
        params.get("subject_info").get("identity_info").get("id_doc_info", {}).get("id_doc_name")
    )
    if id_doc_name:
        params["subject_info"]["identity_info"]["id_doc_info"]["id_doc_name"] = self._core.encrypt(
            id_doc_name
        )
    id_doc_number = (
        params.get("subject_info").get("identity_info").get("id_doc_info", {}).get("id_doc_number")
    )
    if id_doc_number:
        params["subject_info"]["identity_info"]["id_doc_info"][
            "id_doc_number"
        ] = self._core.encrypt(id_doc_number)
    id_doc_address = (
        params.get("subject_info").get("identity_info").get("id_doc_info", {}).get("id_doc_address")
    )
    if id_doc_address:
        params["subject_info"]["identity_info"]["id_doc_info"][
            "id_doc_address"
        ] = self._core.encrypt(id_doc_address)
    ...
    params["bank_account_info"]["account_name"] = self._core.encrypt(
        params["bank_account_info"]["account_name"]
    )
    params["bank_account_info"]["account_number"] = self._core.encrypt(

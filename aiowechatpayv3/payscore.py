...
def payscore_permission_query(self, service_id, authorization_code=None, openid=None):
    ...
    if authorization_code:
        path = "/v3/payscore/permissions/authorization-code/%s?service_id=%s" % (
            authorization_code,
            service_id,
        )
    elif openid:
        path = "/v3/payscore/permissions/openid/%s?appid=%s&service_id=%s" % (
            openid,
            self._appid,
            service_id,
        )

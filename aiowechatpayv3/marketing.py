...
def marketing_partnership_query(
    ...
    if business_type not in ["FAVOR_STOCK", "BUSIFAVOR_STOCK"] or not stock_id:
        raise Exception("invalid value in bussiness_type/stock_id.")
    path = '%sauthorized_data={"business_type":"%s","stock_id":"%s"}' % (
        path,
        business_type,
        stock_id,
    )
    if partner_type == "APPID" and partner_appid:
        ...
        path = '%s&partner={"type":"%s","merchant_id":"%s"}' % (
            path,
            partner_type,
            partner_merchant_id,
        )
...
def marketing_favor_stock_merchant(self, stock_creator_mchid, stock_id, offset=0, limit=50):
    ...
    if stock_id:
        ...
        raise Exception("stock_id is not assigned.")
    if stock_creator_mchid:
        path = "%s?stock_creator_mchid=%s&offset=%s&limit=%s&" % (
            path,
            stock_creator_mchid,
            offset,
            limit,
...
def marketing_busifavor_coupon_detail(self, coupon_code, openid):
    ...
    path = "/v3/marketing/busifavor/users/%s/coupons/%s/appids/%s" % (
        openid,
        coupon_code,
        self._appid,
    )
...
def bank_package_file(self, package_id, bank_type, filepath):
    ...
    mimes = {"TXT": " text/plain", "CSV": "text/csv"}
    ...
    params.update(
        {
            "meta": '{"bank_type":"%s", "filename":"%s", "sha256":"%s"}'
            % (bank_type, filename, sha256(content))
        }
    )

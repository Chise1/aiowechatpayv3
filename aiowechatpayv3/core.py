...
class Core:
    def __init__(
        self, mchid, cert_serial_no, private_key, apiv3_key, cert_dir=None, logger=None, proxy=None
    ):
        self._proxy = proxy
        self._mchid = mchid
    ...
    async def request(
        ...
        headers.update(
            {
                "User-Agent": "wechatpay v3 api python sdk(https://github.com/minibear2021/wechatpayv3)"
            }
        )
        if cipher_data:
            headers.update(
        ...
        if self._cert_dir and os.path.exists(self._cert_dir):
            for file_name in os.listdir(self._cert_dir):
                ...
                if (
                    ...
                    and now <= certificate.not_valid_after
                ):
                    self._certificates.append(certificate)
